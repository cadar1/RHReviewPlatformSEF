...
public class CustomerReviewHController implements Initializable {
    @FXML
    ...
    public void updateDatabase() throws IOException {
        if (verifyData() && verifyname() && verifyreview()) {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("Name", restaurantName.getText());
            jsonObject.put("Review", reviewBox.getText());
            jsonObject.put("Rating", rate.getValue());
            jsonObject.put("Meal", nightsText.getText());
    ...
    public boolean verifyreview(){
        if(reviewBox.getText().isEmpty()){
            errorLabel.setText("Write the review Or go back and choose rating!");
        }
        else {
            return true;
        }
